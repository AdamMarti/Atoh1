...
calculate_jaccard_with_window <- function(vec1, vec2, length1, length2, window_percent = 5) {
  ...
  # Create a set to keep track of unique matches
  unique_matches <- c()
  
  # Calculate matches from vec1 to vec2 and add unique matches
  matched_from_vec1 <- sapply(vec1, function(pos1) {
    matched <- which(abs(vec2 - pos1) <= window_size)
    if (length(matched) > 0) {
      unique_matches <<- unique(c(unique_matches, matched))  # Store unique matches
    }
    return(length(matched) > 0)
  })
  
  # Calculate matches from vec2 to vec1 and add unique matches
  matched_from_vec2 <- sapply(vec2, function(pos2) {
    matched <- which(abs(vec1 - pos2) <= window_size)
    if (length(matched) > 0) {
      unique_matches <<- unique(c(unique_matches, matched))  # Store unique matches
    }
    return(length(matched) > 0)
  ...
  # Count the number of unique matches as the intersection length
  intersect_len <- length(unique_matches)
...
jaccard_to_matrix <- function(jaccard_df) {
  # Convert the Jaccard index data frame into a square matrix
  unique_genes <- unique(c(jaccard_df$Gene1, jaccard_df$Gene2))
  
  jaccard_matrix <- matrix(NA, nrow = length(unique_genes), ncol = length(unique_genes),
                           dimnames = list(unique_genes, unique_genes))
  
  for (i in 1:nrow(jaccard_df)) {
    gene1 <- jaccard_df$Gene1[i]
    gene2 <- jaccard_df$Gene2[i]
    value <- jaccard_df$Jaccard_Index[i]
    jaccard_matrix[gene1, gene2] <- value
    jaccard_matrix[gene2, gene1] <- value
  }
...
# Ensure clustering_non_bHLH_SerThr exists and then create dendrogram_non_bHLH
dendrogram_non_bHLH <- as.dendrogram(clustering_non_bHLH_SerThr)
dendrogram_bHLH <- as.dendrogram(clustering_bHLH_SerThr)
# Adjust font size and margins
par(cex = 0.8, mar = c(5, 4, 4, 12))  # Increase label size and adjust right margin

# Plot the dendrogram for non-bHLH Jaccard Index with adjusted branch lengths
plot(
  dendrogram_non_bHLH,
...
# Adjust font size and margins for bHLH dendrogram
par(cex = 0.8, mar = c(5, 4, 4, 12))  # Increase label size and adjust right margin

# Plot the dendrogram for bHLH Jaccard Index with adjusted branch lengths
plot(
  dendrogram_bHLH,
